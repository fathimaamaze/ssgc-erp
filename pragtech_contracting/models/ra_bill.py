...
class RABill(models.Model):
    ...
    total_advance_recovery_amt_for_current_wo = fields.Float('advance recovery of wo', store=True,
                                                             help='advance recovery amount for current wo')
    total_debit_recovery_amt_for_current_wo = fields.Float('debit recovery of wo', store=True,
                                                           help='debit recovery amount for current wo')
    total_credit_recovery_amt_for_current_wo = fields.Float('credit recovery of wo', store=True,
                                                            help='credit recovery amount for current wo')

    total_payable_amt = fields.Float('Basic Amt', help='This is total payable excluding debit,credit,advance',
                                     store=True)
    final_total_payable = fields.Float('Total Payable', help='This is total payable including debit,credit,advance',
                                       compute='get_total_payable_amt', store=True)
    wo_total_amt = fields.Float('Work Order Amt', compute='get_wo_total_amt', store=True)
    wo_balance_advance = fields.Float('WO Balance Advance', compute='get_balance', help='Balance advance amount',
                                      store=True)
    ...
    tax_line_ids = fields.One2many('account.tax', 'rabill_id', string='Tax Lines', oldname='tax_line',
                                   readonly="state != 'draft'", copy=True)
    ...
    def get_ra_bill_lines(self, workorder):
        ...
        for wo_line in workorder.order_line:
            for task_line in wo_line.payment_schedule_line_ids:
                compl_obj = self.env['work.completion'].search(
                    [('task_id', '=', task_line.task_id.id), ('total_percent', '>', 0),
    ...
    def compute_data(self):
        ...
        if not self.account_invoice_id:
            invoice_vals = []
            my_dict = {self.project_id.analytic_account_id.id: 100.0}
            ...
            self.write({'account_invoice_id': move_id.id})
    ...
    def get_taxes_values(self):
        ...
        for line in self.rabill_line_ids:
            ...
            taxes = \
            line.tax_ids.compute_all(price_unit, self.currency_id, wo_line.quantity, line.task_id, self.contractor_id)[
                'taxes']
            for tax in taxes:
                val = {
                    'bill_id': self.id,
            ...
            if rec.counter == 0:
                if context.get('copy') == True:
                    ...
                    for bill_line in rec.rabill_line_ids:
                        completion_line = rec.env['work.completion.line'].search(
                    ...
                    """ Validation """
                    if rec.final_total_payable < 0 and rec.final_total_payable < (
                            rec.wo_balance_advance + rec.wo_balance_debit + rec.till_date_billed):
                        raise UserError(_('Invalid payable amount'))
    ...
    def create_ra_bill_invoice(self):
        ...
        recovery_sum = sum([line.this_bill_recovery for line in self.advance_recovery_ids]) + sum(
            [line.this_bill_recovery for line in self.debit_note_ids])
...
class RABillLine(models.Model):
    ...
    tax_ids = fields.Many2many('account.tax', 'bill_line_tax', 'bill_id', 'tax_id', string='Taxes',
