...
class LabourRequisition(models.TransientModel):
    ...
    task_category = fields.Many2many('task.category', 'labour_req_task_categ_rel', 'requisition_id', 'task_category_id',
                                     string="Task Category")
    labour_category = fields.Many2many('labour.category', 'labour_req_labour_categ_rel', 'requisition_id',
                                       'labour_category_id', string='Labour Category')
    labour_id = fields.Many2one('labour.master', 'Labour')
    group_id = fields.Many2one('project.task', 'Group')
    task_id = fields.Many2one('project.task', 'Task')
    requisition_line_ids = fields.One2many('labour.requisition.wizard.line', 'requisition_id',
    ...
    task_date_type = fields.Selection([('planned', 'Planned'), ('actual', 'Actual')], string='Task Date Type',
                                      default='planned')  # ,'Search Task having'
    ...
    def add_labour_requisition(self):
        ...
        for line in self.requisition_line_ids:
            if (line.current_order_qty > (
                    line.requisition_qty - line.total_ordered_qty) or line.current_order_qty == 0) and line.is_use == True:
...
class LabourRequisitionWizardLine(models.TransientModel):
    ...
    stage_id = fields.Many2one('transaction.stage', string='Transaction Stage',
                               domain=[('model', '=', 'labour.requisition.wizard.line')])
    work_class = fields.Many2one('labour.work.classification', 'Work Class')
    labour_id = fields.Many2one('labour.master', 'Labour')
    is_use = fields.Boolean(' ')
    task_category = fields.Many2many('task.category', 'labour_req_line_task_categ_rel', 'requisition_line_id',
                                     'task_category_id', string="Task Category")
    labour_category = fields.Many2many('labour.category', 'labour_req_line_labour_categ_rel', 'requisition_line_id',
                                       'labour_category_id', string='Labour Category')
    project_id = fields.Many2one('project.project', related='requisition_id.project_id', store=True, string='Project')
    sub_project = fields.Many2one('sub.project', related='requisition_id.sub_project', string='Sub Project',
                                  required=True)
