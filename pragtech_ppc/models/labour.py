...
class TaskLabourLine(models.Model):
    ...
    name = fields.Char('Labour Estimation No', default="/")
    ...
    task_category = fields.Many2one('task.category', related='labour_line_id.category_id', store=True,
                                    string='Task Category')
    date_start = fields.Datetime(string='Start Date', related='labour_line_id.date_assign', store=True)

    planned_start_date = fields.Datetime(string='Planned Start Date', related='labour_line_id.planed_start_date',
                                         store=True)
    planned_finish_date = fields.Datetime(string='Planned Finish Date', related='labour_line_id.planned_finish_date',
                                          store=True)
    actual_start_date = fields.Date(string='Actual Finish Date', related='labour_line_id.actual_start_date', store=True)
    actual_finish_date = fields.Date(string='Actual Finish Date', related='labour_line_id.actual_finish_date',
                                     store=True)
...
class LabourRequisition(models.Model):
    ...
    balance_qty = fields.Float('Balance Qty', compute='get_balanced_quantity',
                               help="Current requisition qty-Total ordered qty")
    status = fields.Selection([('active', 'Active'), ('inactive', 'Inactive')], 'Status')
...
class LabourRequisitionLine(models.Model):
    ...
    stage_id = fields.Many2one('transaction.stage', string='Transaction Stage',
                               domain=[('model', '=', 'labour.requisition.line')], readonly=True)
    work_class = fields.Many2one('labour.work.classification', 'Work Class')
    labour_id = fields.Many2one('labour.master', 'Labour')
...
class LabourQuotation(models.Model):
    ...
    company_id = fields.Many2one('res.company', 'Company', readonly=True, index=1,
...
class LabourQuotationLine(models.Model):
    ...
    labour_qty = fields.Float(string='Quantity', digits=dp.get_precision('Product Unit of Measure'), required=True,
                              default=1)
    date_planned = fields.Datetime(string='Scheduled Date', select=True)
    taxes_id = fields.Many2many('account.tax', 'labour_quot_line_tax_rel', 'labour_quot_line_id', 'tax_id',
                                string='Taxes')
    ...
    work_class = fields.Many2one('labour.work.classification', 'Work Class')
    order_id = fields.Many2one('labour.quotation', string='Order Reference', select=True, required=True,
                               ondelete='cascade')
