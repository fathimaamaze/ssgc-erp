...
class Task(models.Model):
    ...
    name = fields.Char('WBS Name', required=True)  # Added For Domain Purpose
    ...
    task_library_ids = fields.One2many('project.task.library', 'task_id', string="Project Tasks",
                                       domain="[('is_library_task','=',True)]")

    project_task_library_ids = fields.Many2many('project.task.library', 'task_library_rel', 'task_id',
                                                'task_library_id', string="Tasks",
                                                domain="[('is_library_task','=',True)]")
    project_child_task_library_ids = fields.Many2many('project.task.library', 'child_task_library_rel', 'task_id',
                                                      'task_library_id', domain="[('is_library_task','=',True)]",
                                                      store=True)
    ...
    boq_seq_code = fields.Integer(default=1001)
    ...
    def _compute_delivery_count(self):
        ...
        for rec in self:
            rec.delivery_count = len(picking_obj.search([('wbs_id', '=', rec.id)]))
    ...
    status = fields.Selection([('unplanned', 'Unplanned'), ('non_started', 'Non Started'), ('started', 'Started'),
                               ('in_complete', 'In Complete'), ('completed', 'Completed')], string='Status')
    ...
    def get_actual_cost(self):
        ...
        for task in self:
            ...
            account_move_rec = self.env['account.move'].search(
                [('project_wbs_id', '=', task.id), ('state', '=', 'posted')])
    ...
    def change_state(self, context={}):
        if context.get('copy', False):
            self.flag = True
            # if self.sub_project and self.code in [False, 'New']:
            #     self.code = 'W' + str(self.sub_project.seq_code) + '-' + self.sub_project.code[2:]
            #     self.sub_project.seq_code += 1
        else:
            flag = 0
    ...
    def onchange_project_child_task_library_ids(self):
        ...
        wbs = (self.search([('name', '=', self.name)])).id
        if self.sub_project.budget_applicable:
            category_list = []
            budget_id = self.env['category.budget'].search(
                [('project_id', '=', self.project_id.id), ('project_wbs', '=', wbs),
                    ...
                    if child_task and type(child_task[2]) is dict:
                        if child_task[2]['task_ids']:
                            ...
                            for o in child_task[2]['task_ids']:
                                ...
                                self._cr.execute(
                                    "update project_task set parent_id=%s where id=%s" % (res.id, data[0][0]))
    ...
    def get_material_actual_cost_of_completed(self, category):
        ...
        for move in move_obj:
            if move.task_id.actual_finish_date:
                ...
                supplierinfo = self.env['product.supplierinfo'].search([('product_id', '=', move.product_id.id)],
                                                                       limit=1)
    ...
    def get_balance_budet(self, category):
        return self.get_material_actual_cost_of_completed(category) - self.get_material_actual_cost_of_inprogress(
            category)

    def get_material_balance_estimate(self, category):
        return (self.get_material_estimated_budget(category) - (
                self.get_material_actual_cost_of_completed(category) + self.get_material_actual_cost_of_inprogress(
            category)))

    def get_labour_balance_estimate(self, category):
        return (self.get_labour_estimated_budget(category) - (
                self.get_labour_actual_cost_of_completed(category) + self.get_labour_actual_cost_of_inprogress(
            category)))

    def material_cost_variance(self, category):
        return self.get_completed_work_material_estimate(category) - self.get_material_actual_cost_of_completed(
            category)
    ...
    def action_apply(self):
        ...
        if self.project_id and self.code in [False, 'New']:
            self.code = 'PS' + str(self.project_id.wsb_seq_code) + '-' + self.project_id.code[1:]
            self.project_id.wsb_seq_code += 1

        for obj in self.project_task_library_ids:
            for material in obj.task_material_line:
                material_data = {
                    'material_id': material.material_id.id,
                    'material_uom': material.material_uom.id,
    ...
    def generate_delivery(self):
        return {
            'name': _('Warehouse'),
            'view_mode': 'form',
            'res_model': 'wbs.warehouse.wizard',
            'type': 'ir.actions.act_window',
            'target': 'new'
        }

    def open_deliveries(self):
        picking_ids = self.env['stock.picking'].search([('wbs_id', '=', self.id)])
        if picking_ids:
    ...
    def onchange_min_qty(self):
        for line in self:
            if line.is_task:
                lib_material_qty = [task_mat_line.material_uom_qty for task_mat_line in
                                    line.library_task_id.task_material_line]
                cntr = 0
                for material_line in line.task_material_line:
                    try:
                        if line.min_qty > 0 or line.library_task_id.min_qty > 0.0:
                            material_line.material_uom_qty = (line.min_qty * lib_material_qty[
                                cntr]) / line.library_task_id.min_qty
                            cntr = cntr + 1
                    except:
                        pass

                #   Labour Qty Calculation
                lib_material_qty = [task_lbr_line.labour_uom_qty for task_lbr_line in
                                    line.library_task_id.task_labour_line]
                cntr = 0
                for labour_line in line.task_labour_line:
                    try:
                        if line.min_qty > 0 or line.library_task_id.min_qty > 0.0:
                            labour_line.labour_uom_qty = (line.min_qty * lib_material_qty[
                                cntr]) / line.library_task_id.min_qty
                            cntr = cntr + 1
                    except:
                        pass
    ...
    def create_wbs_task_group(self):
        task_list = []
        for project_task_library_id in self.task_library_ids:
            group_obj = self.env['project.task.library'].browse(project_task_library_id.id)
    ...
    def default_get(self, fields_list):
        ...
        budget_obj = self.env['category.budget'].search([('project_id', '=', self._context.get('default_project_id')),
                                                         ('sub_project', '=', self._context.get('default_sub_project')),
...
class TaskEstimate(models.Model):
    ...
    task_status = fields.Selection([('unstarted', 'Unstarted'), ('started', 'Started'), ('completed', 'Completed')],
                                   'Task Status')
