...
class PurchaseConsumption(models.Model):
    ...
    consumption_ids = fields.One2many('purchase.consumption.line', 'order_id')
    ...
    purchase_ids = fields.Many2many('purchase.order', 'po_consumption_rel1', 'consumption_id', 'purchase_id',
                                    string='Purchase Orders')
    consumption_fulfill = fields.Boolean('Req fulfill', compute='is_consumption_fulfill')
    ...
    material_category = fields.Many2one('product.category', string='Material Category', related='material_id.categ_id',
                                        store=True)
    related_task_category = fields.Many2one('task.category', related='task_id.category_id', store=True)
    task_category = fields.Many2many('task.category', 'purchase_cons_req_task_cat_rel', 'purchase_req_id',
                                     'task_category_id',
    ...
    def get_balanced_qty(self):
        for id in self:
            id.balance_qty = id.quantity - (id.current_consum_qty + id.consumption_as_on_date)

    @api.depends('current_consum_qty')
    @api.onchange('current_consum_qty')
    def validate_req_qty(self):
        self.balance_qty = self.quantity - (self.total_ordered_qty + self.current_consum_qty)
    ...
    def change_state(self, context={}):
        if self.counter == 0:
            ...
            if requisition_till_date <= self.estimation_id.material_uom_qty:
                ...
                raise UserError(_('Sorry you cannot approve consumption greater then available quantity!'))

            if context.get('copy'):
                self.write({'state': 'confirm'})
                if self.product_location_id:

                    # Create Stock moves on Confirmation instead of Creation.
                    location = self.env['stock.location'].search([('usage', '=', 'production')], limit=1)
                    project_location = self.env['stock.location'].search([('project_id', '=', self.project_id.id)],
                                                                         limit=1)
                    loc_id = False
                    loc_dest_id = False
                    ...
                    location_ids = self.env['stock.quant'].search([('product_id.id', '=', self.material_id.id),
                                                                   ('location_id', '=', self.product_location_id.id)])

                    for id in location_ids:
                        id.sudo().reserved_quantity = id.reserved_quantity - (self.current_consum_qty)
...
class PurchaseConsumptionLine(models.Model):
    ...
    consumption_as_on_date = fields.Float('Consumption as on date')
    current_consum_qty = fields.Float('Current consumption Qty')
    order_id = fields.Many2one('purchase.consumption', 'consumption_ids')

    @api.depends('current_consum_qty', 'total_ordered_qty')
    def get_balanced_qty(self):
        self.balance_qty = self.order_id.current_consum_qty - self.order_id.total_ordered_qty
