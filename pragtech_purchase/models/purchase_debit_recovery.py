...
class PurchaseDebitRecovery(models.Model):
    _name = 'purchase.debit'
    _description = 'Purchase Debit Recovery'
    ...
    debit_line_ids = fields.One2many('purchase.debit.line', 'debit_recovery_line_id')

    @api.model_create_multi
    def create(self, vals_list):
        for vals in vals_list:
            rec = super(PurchaseDebitRecovery, self).create(vals)
...
class PurchaseDebitRecoveryLine(models.Model):
    _name = 'purchase.debit.line'
    _description = 'Purchase Debit Recovery line'
    ...
    debit_recovery_line_id = fields.Many2one('purchase.debit', 'Debit Recovery')
