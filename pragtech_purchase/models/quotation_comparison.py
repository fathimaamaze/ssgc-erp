...
class QuotationComparison(models.Model):
    ...
    project_wbs_id = fields.Many2one('project.task', 'Project WBS Name', required=True,
                                     domain=[('is_wbs', '=', True), ('project_id', '!=', False)])

    sub_project = fields.Many2one('sub.project', 'Sub Project', required=True)

    sequence_name = fields.Char('Comparison Reference', required=True, select=True, copy=False, default='New')
    from_date = fields.Datetime('From Date', default=str(date.today() - timedelta(days=15)))
    to_date = fields.Datetime('To Date', default=str(date.today() + timedelta(days=1)))
    material_line = fields.One2many('material.list', 'quotation_comp_mat_id', string='Material Lines', copy=True)
    vendors_line = fields.One2many('supplier.list', 'quotation_comp_sup_id', string='Vendor Lines', copy=True)
    quotation_details = fields.One2many('quotation.details', 'quotation_comp_detail_id', string='Quotation Lines',
                                        copy=True)
    quotation_comp_particular = fields.One2many('quotation.compare.particular', 'quotation_comp_pert_id',
                                                string='Quotation Details Line', copy=True)
    total_amount = fields.Float(string='Total Amount', store=True, compute='compute_total_amount')

    quotation_comp_vendor1 = fields.One2many('quotation.compare.vendor1', 'quotation_comp_ven1_id',
                                             string='Quotation Details Vendor1', copy=True)
    quotation_comp_vendor2 = fields.One2many('quotation.compare.vendor2', 'quotation_comp_ven2_id',
                                             string='Quotation Details Vendor2', copy=True)
    quotation_comp_vendor3 = fields.One2many('quotation.compare.vendor3', 'quotation_comp_ven3_id',
                                             string='Quotation Details Vendor3', copy=True)
    quotation_comp_vendor4 = fields.One2many('quotation.compare.vendor4', 'quotation_comp_ven4_id',
                                             string='Quotation Details Vendor4', copy=True)
    quotation_comp_vendor5 = fields.One2many('quotation.compare.vendor5', 'quotation_comp_ven5_id',
                                             string='Quotation Details Vendor6', copy=True)
    quotation_comp_vendor6 = fields.One2many('quotation.compare.vendor6', 'quotation_comp_ven6_id',
                                             string='Quotation Details Vendor6', copy=True)

    quotation_comp_tax_particular = fields.One2many('tax.particular', 'quotation_comp_taxper_id',
                                                    string='Quotation Tax Particular Details', copy=True)

    quotation_comp_tax_details1 = fields.One2many('tax.details.vendor1', 'quotation_comp_tax1_id',
                                                  string='Quotation Tax1 Details', copy=True)
    quotation_comp_tax_details2 = fields.One2many('tax.details.vendor2', 'quotation_comp_tax2_id',
                                                  string='Quotation Tax2 Details', copy=True)
    quotation_comp_tax_details3 = fields.One2many('tax.details.vendor3', 'quotation_comp_tax3_id',
                                                  string='Quotation Tax3 Details', copy=True)
    quotation_comp_tax_details4 = fields.One2many('tax.details.vendor4', 'quotation_comp_tax4_id',
                                                  string='Quotation Tax4 Details', copy=True)
    quotation_comp_tax_details5 = fields.One2many('tax.details.vendor5', 'quotation_comp_tax5_id',
                                                  string='Quotation Tax5 Details', copy=True)
    quotation_comp_tax_details6 = fields.One2many('tax.details.vendor6', 'quotation_comp_tax6_id',
                                                  string='Quotation Tax6 Details', copy=True)
    ...
    stage_id = fields.Many2one('stage.master', 'Stage', default=_default_stage, copy=False)

    total_vendor1 = fields.Float("Total", store=True, compute='_amount_particular_all')
    total_vendor2 = fields.Float("Total", store=True, compute='_amount_particular_all')
    total_vendor3 = fields.Float("Total", store=True, compute='_amount_particular_all')
    total_vendor4 = fields.Float("Total", store=True, compute='_amount_particular_all')
    total_vendor5 = fields.Float("Total", store=True, compute='_amount_particular_all')
    total_vendor6 = fields.Float("Total", store=True, compute='_amount_particular_all')

    tax_id = fields.Many2many('account.tax', 'quotation_com_tax_rel', 'tax_id', 'quot_com_id', string='Taxes')

    vendor1 = fields.Many2one('res.partner', 'Vendor1', domain=[('supplier', '=', True)])
    vendor2 = fields.Many2one('res.partner', 'Vendor2', domain=[('supplier', '=', True)])
    vendor3 = fields.Many2one('res.partner', 'Vendor3', domain=[('supplier', '=', True)])
    vendor4 = fields.Many2one('res.partner', 'Vendor4', domain=[('supplier', '=', True)])
    vendor5 = fields.Many2one('res.partner', 'Vendor5', domain=[('supplier', '=', True)])
    vendor6 = fields.Many2one('res.partner', 'Vendor6', domain=[('supplier', '=', True)])

    selected_vendors = fields.Integer(compute='get_selected_vendors', string='No Of vendors')
    ...
    mrf_id = fields.Many2one("purchase.requisition")

    product_1 = fields.Many2one('product.product', 'Product1')
    product_2 = fields.Many2one('product.product', 'Product2')
    product_3 = fields.Many2one('product.product', 'Product3')
    product_4 = fields.Many2one('product.product', 'Product4')
    product_5 = fields.Many2one('product.product', 'Product5')
    product_6 = fields.Many2one('product.product', 'Product6')
    ...
    def onchange_project_id(self):
        ...
        return {
            'domain': {
                'sub_project': [('id', 'in', sub_project_ids)]
            }
        }
    ...
    def create_draft_req(self):
        pass

    def confirm_action(self):
        obj = self.browse(self.id)
        obj.sequence_name = self.env['ir.sequence'].next_by_code('quotation.comparison')

        no_of_approve = []
        no_of_products = [particular.product_id for particular in self.quotation_comp_particular if
                          particular.is_approve]

        for vendor1 in self.quotation_comp_vendor1:
            if vendor1.is_approve:
                no_of_approve.append(vendor1.is_approve)
                product_template_obj = self.env['product.template'].search([('name', '=', vendor1.product_id.name)])
                vals = {
                    'partner_id': vendor1.vendor_id.id,
        ...
        for vendor2 in self.quotation_comp_vendor2:
            if vendor2.is_approve:
                ...
                product_template_obj = self.env['product.template'].search([('name', '=', vendor2.product_id.name)])
                vals = {
                    'partner_id': vendor2.vendor_id.id,
        ...
        for vendor3 in self.quotation_comp_vendor3:
            if vendor3.is_approve:
                ...
                product_template_obj = self.env['product.template'].search([('name', '=', vendor3.product_id.name)])
                vals = {
                    'partner_id': vendor3.vendor_id.id,
        ...
        for vendor4 in self.quotation_comp_vendor4:
            if vendor4.is_approve:
                ...
                product_template_obj = self.env['product.template'].search([('name', '=', vendor4.product_id.name)])
                vals = {
                    'partner_id': vendor4.vendor_id.id,
        ...
        for vendor5 in self.quotation_comp_vendor5:
            if vendor5.is_approve:
                ...
                product_template_obj = self.env['product.template'].search([('name', '=', vendor5.product_id.name)])
                vals = {
                    'partner_id': vendor5.vendor_id.id,
        ...
        for vendor6 in self.quotation_comp_vendor6:
            if vendor6.is_approve:
                ...
                product_template_obj = self.env['product.template'].search([('name', '=', vendor6.product_id.name)])
                vals = {
                    'partner_id': vendor6.vendor_id.id,
        ...
        if no_of_products and no_of_approve:
            self.state = 'confirm'
        else:
    ...
    @api.onchange('mrf_id', 'from_date', 'to_date')
    def onchange_project_wbs1(self):
        # Set material list
        material_ids = self.mrf_id.pr_line_ids.mapped('material_id').ids
        # material_ids = list(set([material.material_id.id for material in self.project_wbs_id.material_estimate_line]))
        material_lines = []

        for material_id in material_ids:
            material_lines.append((0, 0, {'material_id': material_id, 'quotation_comp_mat_id': self.id}))

        # Set vendor list
        quotation_lines = self.env['vendor.quotation.line'].search(
            [('product_id', 'in', material_ids), ('order_id.date_order', '>=', self.from_date),
             ('order_id.date_order', '<=', self.to_date)])

        vendor_ids = quotation_lines.mapped('order_id.partner_id')
        # vendor_ids = list(set([line.order_id.partner_id.id for line in quotation_lines]))
        vendor_lines = []

        for vendor in vendor_ids:
            # vendor_obj = self.env['res.partner'].browse(vendor)
            if vendor.vendor_status == 'suspended':
                vendor_ids.remove(vendor.id)
            if vendor.vendor_status == 'black_listed':
                vendor_ids.remove(vendor.id)

        for vendor_id in vendor_ids:
            vendor_lines.append((0, 0, {'vendor_id': vendor_id, 'quotation_comp_sup_id': self.id}))
    ...
    #     # Set material list
    #     for rec in self:
    #         material_ids = rec.mrf_id.pr_line_ids.mapped('material_id')
    #         material_lines = [(5, 0)]
    #
    #         for material_id in material_ids:
    #             material_lines.append((0, 0, {'material_id': material_id.id}))
    #
    #         # Set vendor list
    #         seller_ids = material_ids.mapped("seller_ids")
    #         vendor_lines = [(5, 0)]
    #         for vendor_id in seller_ids:
    #             vendor_lines.append((0, 0, {'vendor_id': vendor_id.partner_id.id}))
    #
    #         self.update({
    #             'material_line': material_lines,
    #             'vendors_line': vendor_lines,
    ...
    @api.depends('quotation_comp_vendor1.amount', 'quotation_comp_vendor2.amount', 'quotation_comp_vendor3.amount',
                 'quotation_comp_vendor4.amount', 'quotation_comp_vendor5.amount', 'quotation_comp_vendor6.amount',
                 'quotation_comp_tax_details1.tax', 'quotation_comp_tax_details2.tax',
                 'quotation_comp_tax_details3.tax',
                 'quotation_comp_tax_details4.tax', 'quotation_comp_tax_details5.tax',
                 'quotation_comp_tax_details6.tax')
    ...
    def compute_quotation_details(self):
        ...
        quotation_ids = self.env['vendor.quotation'].search(
            [('date_order', '>=', self.from_date), ('state', '=', 'confirm'),
             ('date_order', '<=', self.to_date), ('partner_id', 'in', selected_vendor_ids)])
            ...
            for quotation_line in quotation.order_line:
                if quotation_line.product_id.id in material_ids:
                    ...
                    for line in self.quotation_details:
                        if line.vendor_id.id == quotation_line.partner_id.id and line.name == quotation.name and line.product_id.id == quotation_line.product_id.id:
                            old_line_flag = True
                            old_line = line
                            break
        ...
        for idx, product_id in enumerate(material_ids, 1):
            product = 'product_' + str(idx)
            val.update({product: product_id, })

        # If selected vendor is less than set other header to None
        if len(selected_vendor_ids) < 6:
            for idx in range(len(material_ids) + 1, 7):
                product = 'product_' + str(idx)
                val.update({product: None, })
            ...
            for vendor in self.quotation_comp_vendor1:
                for tax_line in vendor.tax_id:
                    if tax_line in tax_amount:
                        tax_amount[tax_line] += (tax_line.amount / 100) * vendor.amount
                        ...
                        tax_amount.update({tax_line: (tax_line.amount / 100) * vendor.amount})
    ...
    def compute_selected_quotations(self):
        ...
        if self.quotation_details:
            ...
            tax_list = []
            material_lst = []
            selected_vendor_ids = [line.vendor_id.id for line in self.vendors_line if line.is_vendor]
            ...
            for line in self.quotation_details:
                ...
                if line.is_use:
                    ...
                    # tax Particular 1
                    if not self.env['tax.particular'].search(
                            [('product_id', '=', line.product_id.id), ('quotation_comp_taxper_id', '=', self.id)]):
        ...
        for line in self.material_line:
            if line.is_material:
                ...
                particulat_prod_list = [i.product_id.id for i in self.quotation_comp_particular]
        ...
        try:
            ...
            for i in range(1, 7):
                if hasattr(self, 'product_' + str(i)):
                    product = getattr(self, 'product_' + str(i))
                    if product:
                        lst = []
                        for rec in self.quotation_details.filtered(lambda x: x.product_id.id == product.id):
                            vals = {
                                'product_id': rec.product_id.id,
                                'vendor_id': rec.vendor_id.id,
                                'amount': rec.price_unit,
                                'negotiated_rate': rec.price_unit,
                                'quotation_comp_ven' + str(i) + '_id': self.id,
                            }
                            lst.append((0, 0, vals))

                        self.update({'quotation_comp_vendor' + str(i): lst})

            # quotation_compare_vendor1_list = []
            # vals = {}
            # old_lines = [line.product_id.id for line in self.quotation_comp_vendor1]
            ...
            #                 [('product_id', '=', line.product_id.id), ('vendor_id', '=', None),
            #                  ('quotation_comp_ven1_id', '=', self.id)])
            #             if not blank_line1:
            #                 vals = {
            #                     'product_id': line.product_id.id,
            ...
            #             quotation_compare_vendor1_list.append((2, quotation_comp_vendor1.id))
            #
            #     self.update({'quotation_comp_vendor1': quotation_compare_vendor1_list})
            #
            ...
            #             quotation_compare_vendor2_list.append((2, quotation_comp_vendor2.id))
            #
            #     self.update({'quotation_comp_vendor2': quotation_compare_vendor2_list})
            #
            # vals = {}
            ...
            #             quotation_compare_vendor3_list.append((2, quotation_comp_vendor3.id))
            #
            #     self.update({'quotation_comp_vendor3': quotation_compare_vendor3_list})
            #
            ...
            #             quotation_compare_vendor4_list.append((2, quotation_comp_vendor4.id))
            #
            #     self.update({'quotation_comp_vendor4': quotation_compare_vendor4_list})
            #
            ...
            #             quotation_compare_vendor5_list.append((2, quotation_comp_vendor5.id))
            #
            #     self.update({'quotation_comp_vendor5': quotation_compare_vendor5_list})
            #
            ...
            #             quotation_compare_vendor6_list.append((2, quotation_comp_vendor6.id))
            #
            #     self.update({'quotation_comp_vendor6': quotation_compare_vendor6_list})

        except IndexError:
            pass

    @api.depends('quotation_comp_vendor1.negotiated_rate', 'quotation_comp_vendor1.amount',
                 'quotation_comp_particular.product_qty')
    ...
    @api.depends('quotation_comp_vendor2.negotiated_rate', 'quotation_comp_vendor2.amount',
                 'quotation_comp_particular.product_qty')
    ...
    @api.depends('quotation_comp_vendor3.negotiated_rate', 'quotation_comp_vendor3.amount',
                 'quotation_comp_particular.product_qty')
    ...
    @api.depends('quotation_comp_vendor4.negotiated_rate', 'quotation_comp_vendor4.amount',
                 'quotation_comp_particular.product_qty')
    ...
    @api.depends('quotation_comp_vendor5.negotiated_rate', 'quotation_comp_vendor5.amount',
                 'quotation_comp_particular.product_qty')
    ...
    @api.depends('quotation_comp_vendor6.negotiated_rate', 'quotation_comp_vendor6.amount',
                 'quotation_comp_particular.product_qty')
...
class MaterialList(models.Model):
    ...
    quotation_comp_mat_id = fields.Many2one('quotation.comparison', 'Quotation')
...
class SupplierList(models.Model):
    ...
    quotation_comp_sup_id = fields.Many2one('quotation.comparison', 'Quotation')
...
class QuotationDetails(models.Model):
    ...
    tax_id = fields.Many2many('account.tax', 'quotation_detail_tax_rel', 'tax_id', 'quot_detail_id', string='Taxes')
    tax_percent = fields.Float('Tax Percent')
    tax = fields.Float('Total Tax')

    quotation_comp_detail_id = fields.Many2one('quotation.comparison', 'Quotation')

    quotation_details_ = fields.One2many('quotation.compare.particular', 'quotation_comp_pert_detail_id',
                                         string='Quotation Details Line')
    quotation_details_ven1 = fields.One2many('quotation.compare.vendor1', 'quotation_comp_ven1_detail_id',
                                             string='Quotation Details Vendor1')
    quotation_details_ven2 = fields.One2many('quotation.compare.vendor2', 'quotation_comp_ven2_detail_id',
                                             string='Quotation Details Vendor2')
    quotation_details_ven3 = fields.One2many('quotation.compare.vendor3', 'quotation_comp_ven3_detail_id',
                                             string='Quotation Details Vendor3')
    quotation_details_ven4 = fields.One2many('quotation.compare.vendor4', 'quotation_comp_ven4_detail_id',
                                             string='Quotation Details Vendor4')
    quotation_details_ven5 = fields.One2many('quotation.compare.vendor5', 'quotation_comp_ven5_detail_id',
                                             string='Quotation Details Vendor5')
    quotation_details_ven6 = fields.One2many('quotation.compare.vendor6', 'quotation_comp_ven6_detail_id',
                                             string='Quotation Details Vendor6')

    quotation_tax_per = fields.One2many('tax.particular', 'quotation_comp_taxper_detail_id',
                                        string='Quotation Particular Tax')

    quotation_tax1_detail_per = fields.One2many('tax.details.vendor1', 'quotation_comp_tax1_detail_id',
                                                string='Quotation Detail Tax1')
    quotation_tax2_detail_per = fields.One2many('tax.details.vendor2', 'quotation_comp_tax2_detail_id',
                                                string='Quotation Detail Tax2')
    quotation_tax3_detail_per = fields.One2many('tax.details.vendor3', 'quotation_comp_tax3_detail_id',
                                                string='Quotation Detail Tax3')
    quotation_tax4_detail_per = fields.One2many('tax.details.vendor4', 'quotation_comp_tax4_detail_id',
                                                string='Quotation Detail Tax4')
    quotation_tax5_detail_per = fields.One2many('tax.details.vendor5', 'quotation_comp_tax5_detail_id',
                                                string='Quotation Detail Tax5')
    quotation_tax6_detail_per = fields.One2many('tax.details.vendor6', 'quotation_comp_tax6_detail_id',
                                                string='Quotation Detail Tax6')
...
class QuotationCompareParticular(models.Model):
    ...
    product_uom = fields.Many2one('uom.uom', string='Unit')
    quotation_comp_pert_id = fields.Many2one('quotation.comparison', 'Quotation')
    quotation_comp_pert_detail_id = fields.Many2one('quotation.details', string='Quotation details', ondelete='cascade')
...
class QuotationCompareVendor1(models.Model):
    ...
    tax_id = fields.Many2many('account.tax', 'quotation_com1_tax_rel', 'tax_id', 'quot_comp1_id', string='Taxes')
    currency_id = fields.Many2one('res.currency', 'Currency')
    amount = fields.Float('Amount')
    quotation_comp_ven1_id = fields.Many2one('quotation.comparison', 'Quotation')
    quotation_comp_ven1_detail_id = fields.Many2one('quotation.details', string='Quotation details', ondelete='cascade')
...
class QuotationCompareVendor2(models.Model):
    ...
    product_id = fields.Many2one('product.product', 'Particular')
    tax_id = fields.Many2many('account.tax', 'quotation_comp2_tax_rel', 'tax_id', 'quot_comp2_id', string='Taxes')
    vendor_id = fields.Many2one('res.partner', 'Vendor', domain=[('supplier', '=', True)])
    negotiated_rate = fields.Float('Nego.Rate')
    currency_id = fields.Many2one('res.currency', 'Currency')
    amount = fields.Float('Amount')
    quotation_comp_ven2_id = fields.Many2one('quotation.comparison', 'Quotation')
    quotation_comp_ven2_detail_id = fields.Many2one('quotation.details', string='Quotation details', ondelete='cascade')
    is_approve = fields.Boolean('Approve')
...
class QuotationCompareVendor3(models.Model):
    ...
    product_id = fields.Many2one('product.product', 'Particular')
    tax_id = fields.Many2many('account.tax', 'quotation_comp3_tax_rel', 'tax_id', 'quot_comp3_id', string='Taxes')
    vendor_id = fields.Many2one('res.partner', 'Vendor', domain=[('supplier', '=', True)])
    negotiated_rate = fields.Float('Nego.Rate')
    amount = fields.Float('Amount')
    quotation_comp_ven3_id = fields.Many2one('quotation.comparison', 'Quotation')
    quotation_comp_ven3_detail_id = fields.Many2one('quotation.details', string='Quotation details', ondelete='cascade')
    is_approve = fields.Boolean('Approve')
...
class QuotationCompareVendor4(models.Model):
    ...
    product_id = fields.Many2one('product.product', 'Particular')
    tax_id = fields.Many2many('account.tax', 'quotation_comp4_tax_rel', 'tax_id', 'quot_comp4_id', string='Taxes')
    vendor_id = fields.Many2one('res.partner', 'Vendor', domain=[('supplier', '=', True)])
    negotiated_rate = fields.Float('Nego.Rate')
    currency_id = fields.Many2one('res.currency', 'Currency')
    amount = fields.Float('Amount')
    quotation_comp_ven4_id = fields.Many2one('quotation.comparison', 'Quotation')
    quotation_comp_ven4_detail_id = fields.Many2one('quotation.details', string='Quotation details', ondelete='cascade')
    is_approve = fields.Boolean('Approve')
...
class QuotationCompareVendor5(models.Model):
    ...
    product_id = fields.Many2one('product.product', 'Particular')
    tax_id = fields.Many2many('account.tax', 'quotation_comp5_tax_rel', 'tax_id', 'quot_comp5_id', string='Taxes')
    vendor_id = fields.Many2one('res.partner', 'Vendor', domain=[('supplier', '=', True)])
    negotiated_rate = fields.Float('Nego.Rate')
    currency_id = fields.Many2one('res.currency', 'Currency')
    amount = fields.Float('Amount')
    quotation_comp_ven5_id = fields.Many2one('quotation.comparison', 'Quotation')
    quotation_comp_ven5_detail_id = fields.Many2one('quotation.details', string='Quotation details', ondelete='cascade')
    is_approve = fields.Boolean('Approve')
...
class QuotationCompareVendor6(models.Model):
    ...
    product_id = fields.Many2one('product.product', 'Particular')
    tax_id = fields.Many2many('account.tax', 'quotation_comp6_tax_rel', 'tax_id', 'quot_comp6_id', string='Taxes')
    vendor_id = fields.Many2one('res.partner', 'Vendor', domain=[('supplier', '=', True)])
    negotiated_rate = fields.Float('Nego.Rate')
    currency_id = fields.Many2one('res.currency', 'Currency')
    amount = fields.Float('Amount')
    quotation_comp_ven6_id = fields.Many2one('quotation.comparison', 'Quotation')
    quotation_comp_ven6_detail_id = fields.Many2one('quotation.details', string='Quotation details', ondelete='cascade')
    is_approve = fields.Boolean('Approve')
...
class TaxParticular(models.Model):
    ...
    product_id = fields.Many2one('product.product', 'product')
    quotation_comp_taxper_id = fields.Many2one('quotation.comparison', 'Quotation')
    quotation_comp_taxper_detail_id = fields.Many2one('quotation.details', string='Quotation details',
                                                      ondelete='cascade')
...
class TaxDetailsVendor1(models.Model):
    ...
    vendor_id = fields.Many2one('res.partner', 'Vendor', domain=[('supplier', '=', True)])
    product_id = fields.Many2one('product.product', 'product')
    tax_id = fields.Many2many('account.tax', 'quotation_comp1_tax_rel', 'tax_id', 'quot_comp1_id', string='Taxes')
    tax = fields.Float('Tax')
    quotation_comp_tax1_id = fields.Many2one('quotation.comparison', 'Quotation')
    quotation_comp_tax1_detail_id = fields.Many2one('quotation.details', string='Quotation details', ondelete='cascade')
...
class TaxDetailsVendor2(models.Model):
    ...
    vendor_id = fields.Many2one('res.partner', 'Vendor', domain=[('supplier', '=', True)])
    product_id = fields.Many2one('product.product', 'product')
    tax_id = fields.Many2many('account.tax', 'tax_detail2_tax_rel', 'tax_id', 'tax_detail_vendor2_id', string='Taxes')
    tax = fields.Float('Tax')
    quotation_comp_tax2_id = fields.Many2one('quotation.comparison', 'Quotation')
    quotation_comp_tax2_detail_id = fields.Many2one('quotation.details', string='Quotation details', ondelete='cascade')
...
class TaxDetailsVendor3(models.Model):
    ...
    vendor_id = fields.Many2one('res.partner', 'Vendor', domain=[('supplier', '=', True)])
    product_id = fields.Many2one('product.product', 'product')
    tax_id = fields.Many2many('account.tax', 'tax_detail3_tax_rel', 'tax_id', 'tax_detail_vendor3_id', string='Taxes')
    tax = fields.Float('Tax')
    quotation_comp_tax3_id = fields.Many2one('quotation.comparison', 'Quotation')
    quotation_comp_tax3_detail_id = fields.Many2one('quotation.details', string='Quotation details', ondelete='cascade')
...
class TaxDetailsVendor4(models.Model):
    ...
    vendor_id = fields.Many2one('res.partner', 'Vendor', domain=[('supplier', '=', True)])
    product_id = fields.Many2one('product.product', 'product')
    tax_id = fields.Many2many('account.tax', 'tax_detail4_tax_rel', 'tax_id', 'tax_detail_vendor4_id', string='Taxes')
    tax = fields.Float('Tax')
    quotation_comp_tax4_id = fields.Many2one('quotation.comparison', 'Quotation')
    quotation_comp_tax4_detail_id = fields.Many2one('quotation.details', string='Quotation details', ondelete='cascade')
...
class TaxDetailsVendor5(models.Model):
    ...
    vendor_id = fields.Many2one('res.partner', 'Vendor', domain=[('supplier', '=', True)])
    product_id = fields.Many2one('product.product', 'product')
    tax_id = fields.Many2many('account.tax', 'tax_detail5_tax_rel', 'tax_id', 'tax_detail_vendor5_id', string='Taxes')
    tax = fields.Float('Tax')
    quotation_comp_tax5_id = fields.Many2one('quotation.comparison', 'Quotation')
    quotation_comp_tax5_detail_id = fields.Many2one('quotation.details', string='Quotation details', ondelete='cascade')
...
class TaxDetailsVendor6(models.Model):
    ...
    vendor_id = fields.Many2one('res.partner', 'Vendor', domain=[('supplier', '=', True)])
    product_id = fields.Many2one('product.product', 'product')
    tax_id = fields.Many2many('account.tax', 'tax_detail6_tax_rel', 'tax_id', 'tax_detail_vendor6_id', string='Taxes')
    tax = fields.Float('Tax')
    quotation_comp_tax6_id = fields.Many2one('quotation.comparison', 'Quotation')
    quotation_comp_tax6_detail_id = fields.Many2one('quotation.details', string='Quotation details', ondelete='cascade')
