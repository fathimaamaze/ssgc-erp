...
class Procurement(models.TransientModel):
    ...
    project_wbs = Many2one('project.task', domain=[('is_wbs', '=', True), ('is_task', '=', False)], required=False)

    sub_project = fields.Many2one('sub.project', 'Sub Project')

    material_id = Many2one('product.product', 'Material Name')
    material = fields.Selection(
        [('All Material', 'All Material'), ('Show All Material from project', 'Show All Material from project'),
         ('Single Material', 'single Material')])
    material_selection = fields.Selection(
        [('all', 'All Material'), ('from_single_project', 'Show All Material from project'),
         ('single', 'single Material')], default='from_single_project')
    from_date = fields.Date('From  Date', default=str(datetime.now() + timedelta(days=-7)), required=True)
    to_date = fields.Date('To Date', default=str(fields.date.today()), required=True)
    ...
    procurement_line_ids = fields.One2many('procurement.line', 'procurement_id', string='Requisition Order')
    warehouse_id = fields.Many2one('stock.warehouse', 'procurement Type', required=True)
    ...
    def compute_procurements_lines(self):
        ...
        if self.material_selection == 'all':
            ...
            requisition_line_obj = self.env['purchase.requisition'].search(domain)
