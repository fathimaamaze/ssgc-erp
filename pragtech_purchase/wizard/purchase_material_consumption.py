...
class PurchaseConsumptionWizard(models.TransientModel):
    ...
    def get_task_idss(self):
        self.task_id_list = []
        var = ''
        material_estimate_obj = self.env['task.material.line'].search([('wbs_id', '=', self.name.id)])
    ...
    note = fields.Text('Note:', default='You can only add consumption for approved material for current vendor')
    name = fields.Many2one('project.task', 'Project WBS Name', domain=[('is_wbs', '=', True), ('is_task', '=', False)],
                           required=True)
    project_id = fields.Many2one('project.project', 'Project', required=True,
                                 domain=[('approval_state', '=', 'approve')])
    sub_project = fields.Many2one('sub.project', 'Sub Project', required=True,
                                  domain=[('approval_state', '=', 'approve')])
    task_category = fields.Many2many('task.category', 'consumption_cat_wizard_rel', 'cat_id', 'wizard_id')
    material_category = fields.Many2many('product.category', 'cons_wizard_prod_cat_rel', 'cat_id', 'wizard_id')
    material = fields.Many2one('product.product', 'Material')
    group_id = fields.Many2one('project.task', 'Task Group')
    task_id = fields.Many2one('project.task', 'Task')
    consumption_ids = fields.One2many('purchase.consumption.wizard.line', 'order_id')
    to_date = fields.Date('To Date', default=fields.date.today(), required=True)
    from_date = fields.Date('From Date', default=(datetime.now() + timedelta(days=-30)).date(), required=True)
    ...
    is_red = fields.Boolean()  # line will read if Material rate is not approved
    task_ids_str = []
    ...
    def group_id_onchange(self):
        ...
        return {
            'domain': {
                'task_id': [('parent_id', '=', self.group_id.id), ('is_task', '=', True),
    ...
    def task_category_onchange(self):
        return {
            'domain': {'task_id': [('category_id', '=', self.task_category.id), ('is_task', '=', True),
                                   ('project_wbs_id', '=', self.name.id)]}
        }

    def get_is_red(self, material_id, partner_id):
        product_id = self.env['product.product'].search([('id', '=', material_id)])
    ...
    @api.depends('project_id', 'sub_project', 'name', 'group_id', 'task_category', 'task_id', 'material_category',
                 'material')
    def compute_consumptions_lines(self):
        ...
        project_task_obj = self.env['project.task'].search(
            [('project_id', '=', self.project_id.id), ('name', '=', self.name.name),
        ...
        for line in material_estimate_obj:
            ...
            consumption_records = self.env['purchase.consumption'].search(
                [('material_id', '=', line.material_id.id), ('project_id', '=', self.project_id.id),
                 ('sub_project', '=', self.sub_project.id), ('project_wbs', '=', self.name.id),
                 ('state', '=', 'confirm')])
            ...
            consumption = self.consumption_ids.create(vals)
            consumption.onchange_product_location_id()

        view_id = self.env.ref('pragtech_purchase.purchase_consumption_wizard_view').id
        return {
    ...
    def create_consumptions(self):
        ...
        view_id = self.env.ref('pragtech_purchase.purchase_consumption_tree_view').id
        return {
            'name': 'Consumption',
            'view_type': 'form',
            'view_mode': 'tree, form',
    ...
    def search_consumptions_on_po(self):
        ...
        purchase_consumption_obj = self.env['purchase.consumption'].search(domain)
        for line in purchase_consumption_obj:
            ...
            if (line.current_consum_qty - line.total_ordered_qty) > 0:
                price_info = self.get_is_red(line.material_id.id, Purchase_order_obj.partner_id.id)
        ...
        view_id = self.env.ref('pragtech_purchase.consumption_wizard_for_po').id
        return {
    ...
    def add_consumption(self):
        ...
        old_po_consumption_lines = [line.consumption_id for line in po_consumption_line_obj]
        vals = {}
        for line in self.consumption_ids:
            if line.is_use:
                if line.current_order_qty > (line.consumption_qty - line.total_ordered_qty):
                    raise UserError(_('Order quantity must be less than consumption quantity.'))

                if line.current_order_qty == 0:
                    raise UserError(_('Please enter valid consumption Quantity.'))

                if line.is_red and line.is_use:
                    raise UserError('You can only add consumption for approved material for current vendor.')
...
class PurchaseConsumptionWizardLine(models.TransientModel):
    ...
    consumption_date = fields.Date('Date', default=fields.date.today(), required=True)
    ...
    current_consum_qty = fields.Float('Current consumption Qty', readonly=False)
    ...
    consumption_type = fields.Selection([('estimated', 'Estimated'), ('non_estimated', 'Non Estimated')], 'Type')
    order_id = fields.Many2one('purchase.consumption.wizard', 'consumption_ids')
    ...
    stage_id = fields.Many2one('transaction.stage', domain=[('model', '=', 'consumption.order.line')])
    project_wbs = fields.Many2one('project.task', related='order_id.name', store=True)
    project_id = fields.Many2one('project.project', related='order_id.project_id', store=True)

    sub_project = fields.Many2one('sub.project', 'Sub Project', related='order_id.sub_project', store=True)

    material_category = fields.Many2many('product.category', 'cons_wizard_prod_cat_rels', 'cat_id', 'wizard_line_id')
    task_category = fields.Many2many('task.category', 'consumption_cat_wizard_rels', 'cat_id', 'wizard_id')
    ...
    def onchange_product_location_id(self):
        ...
        location_id_list = []
        if self.material_id:
            location_ids = stock_quant_id.search([('product_id.id', '=', self.material_id.id)])
            ...
            return {
                'domain': {
                    'product_location_id': [('id', 'in', location_id_list), ('usage', '=', 'internal')]
                }
            }
        return {
            'domain': {
                'product_location_id': [('id', 'in', [])]
            }
        }
