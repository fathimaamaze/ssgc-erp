...
class PurchaseRequisitionWizard(models.TransientModel):
    ...
    note = fields.Text('Note:', default='You can only add Requisition for approved material for current vendor')
    ...
    sub_project = fields.Many2one('sub.project', 'Sub Project', required=True,
                                  domain=[('approval_state', '=', 'approve')])
    task_category = fields.Many2many('task.category', 'cat_wizard_rel', 'cat_id', 'wizard_id')
    material = fields.Many2one('product.product', 'Material')
    group_id = fields.Many2one('project.task', 'Task Group')
    task_id = fields.Many2one('project.task', 'Task')
    requisition_ids = fields.One2many('purchase.requisition.wizard.line', 'order_id')
    ...
    def task_category_onchange(self):
        return {
            'domain': {
                'task_id': [('category_id', '=', self.task_category.id), ('is_task', '=', True),
                            ('project_wbs_id', '=', self.name.id)]
            }
        }
    ...
    def get_is_red(self, material_id, partner_id):
        price_info = self.env['product.supplierinfo'].search(
            [('product_tmpl_id', '=', material_id), ('is_active', '=', True), ('partner_id', '=', partner_id)])
    ...
    @api.depends('project_id', 'sub_project', 'name', 'group_id', 'task_category', 'task_id',
                 'material')
    def compute_requisitions_lines(self):
        ...
        for line in material_estimate_obj:
            if not line.material_line_id.actual_finish_date and line.balanced_requisition > 0:
                ...
                self.requisition_ids.create(vals)

        view_id = self.env.ref('pragtech_purchase.purchase_requisition_wizard_view').id
        return {
    ...
    def create_requisitions(self):
        ...
        if lst:
            vals = {
                ...
                'task_category': self.task_id.category_id,
    ...
    @api.depends('project_id', 'name', 'material', 'to_date', 'from_date')
    def search_requisitions_on_po(self):
        ...
        purchase_requisition_obj = self.env['purchase.requisition'].search(domain)
        ...
        view_id = self.env.ref('pragtech_purchase.requisition_wizard_for_po').id
        return {
    ...
    def add_requisitions(self):
        po_requisition_list = []
        Purchase_order_obj = self.env['purchase.order'].browse(self._context.get('active_id'))
        po_requisition_line_obj = self.env['po.requisition'].search([('order_id', '=', self._context.get('active_id'))])
        old_po_requisition_lines = [line.requisition_id for line in po_requisition_line_obj]
        vals = {}
        ...
        for line in self.requisition_ids:
            if line.is_use:
                ...
                if line.is_red and line.is_use == False:
                    raise UserError('You can only add Requisition for approved material for current vendor.')

                if line.requisition_name not in old_po_requisition_lines and line.current_order_qty > 0 and line.current_order_qty <= (
                        line.requisition_qty - line.total_ordered_qty):
                    vals = {
                        'project_id': self.project_id.id,
                        'project_wbs': self.name.id,
                        'sub_project': self.sub_project.id,
...
class PurchaseRequisitionWizardLine(models.TransientModel):
    ...
    order_id = fields.Many2one('purchase.requisition.wizard', 'requisition_ids')
    ...
    task_category = fields.Many2many('task.category', 'cat_wizard_rels', 'cat_id', 'wizard_id')
