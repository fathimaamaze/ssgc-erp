...
class WorkOrder(models.Model):
    ...
    labour_category = fields.Many2many('labour.category', 'wo_category_rel', 'work_order_id', 'category_id',
    ...
    order_line = fields.One2many('work.order.line', 'order_id', string='Order Lines', copy=True, ondelete='cascade',
                                 domain=[('quantity', '>', 0)])
    ...
    state = fields.Selection([('draft', 'Draft'), ('confirm', 'Confirm')], string='Status', copy=False, index=True,
    ...
    company_id = fields.Many2one('res.company', 'Company', required=True, index=1,
    ...
    def create_order_lines(self, work_order_obj, project_wbs, stage_id):
        ...
        for line in my_list:
            ...
            contractor_price = self.env['labour.contractorinfo'].search(
                [('labour_id', '=', lbr_obj.id), ('name', '=', work_order_obj.partner_id.name),
                 ('is_active', '=', True)])
            if contractor_price:
                rate = contractor_price.price
            else:
                lbr_estimate = self.env['task.labour.line'].search(
                    [('labour_line_id', '=', line.get('task_id')), ('labour_id', '=', line.get('labour_id'))], limit=1)
            ...
            if task_labour in old_order_lines and not self.stage_id.approved:
                ...
                for wo_req in self.wo_requisition_line:
                    if wo_req.labour_id.id == task_labour.get('labour_id') and wo_req.task_id.id == task_labour.get(
                            'task_id'):
                        ...
                        for order_line in work_order_obj.order_line:
                            if order_line.labour_id.id == task_labour.get(
                                    'labour_id') and order_line.task_id.id == task_labour.get('task_id'):
                                order_line.write({
