...
class CategoryBudget(models.Model):
    ...
    def create(self, vals_list):
        for vals in vals_list:
            obj = self.search(
                [('project_id', '=', vals.get('project_id')), ('sub_project', '=', vals.get('sub_project')),
...
class CategoryBudgetLine(models.Model):
    ...
    def compute_labour_material_cost(self):
        ...
        for line in self:
            ...
            labour_obj = self.env['task.labour.line'].search(
                [('task_category', '=', line.task_category.id), ('wbs_id', '=', wbs_obj.id)])
            for labour in labour_obj:
                labour_total += labour.labour_rate
                line.labour_cost = labour_total
            material_obj = self.env['task.material.line'].search(
                [('task_category', '=', line.task_category.id), ('wbs_id', '=', wbs_obj.id)])
