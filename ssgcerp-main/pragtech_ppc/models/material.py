...
class TaskMaterialLine(models.Model):
    ...
    name = fields.Char('Material Estimation No', default='/')
    ...
    material_category = fields.Many2one('product.category', related='material_id.categ_id', store=True,
                                        string='Material Category')
    task_category = fields.Many2one('task.category', related='material_line_id.category_id', store=True,
                                    string='Task Category')
    planned_start_date = fields.Datetime(string='Planned Start Date', related='material_line_id.planed_start_date',
                                         store=True)
    planned_finish_date = fields.Datetime(string='Planned Finish Date', related='material_line_id.planned_finish_date',
                                          store=True)
    actual_start_date = fields.Date(string='Actual Finish Date', related='material_line_id.actual_start_date',
                                    store=True)
    actual_finish_date = fields.Date(string='Actual Finish Date', related='material_line_id.actual_finish_date',
                                     store=True)
    sequence = fields.Char()

    delivered_qty = fields.Float('Delivery Qty')
    ...
    def create(self, vals_list):
        for vals in vals_list:
            ...
            # sme'] = self.env['ir.sequence'].next_by_code('task.material.line') or '/'
