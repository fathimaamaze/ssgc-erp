...
class TaskScheduler(models.TransientModel):
    ...
    start_finish_dates = fields.Selection([('start_date', 'Start Date'), ('finish_date', 'Finish Date')], 'Date',
                                          default='start_date')
    ...
    note = fields.Text('Note:',
                       default='Completion (%) and actual finish date can be updated for only non billable tasks')
                        ...
                        for line in self.scheduler_line_ids:
                            if line.is_use:
                                if line.task_id.is_billable:
                                    raise UserError(
                                        _('You cannot update % completion or actual finish date for billable tasks.'))
                                else:
