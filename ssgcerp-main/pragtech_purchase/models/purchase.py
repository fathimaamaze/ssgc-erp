...
class PurchaseOrder(models.Model):
    ...
    # is_quantity_copy = fields.Char('Quantity Copy')
    ...
    project_wbs = fields.Many2one('project.task', 'Project WBS Name',
    ...
    transaction_count = fields.Integer('Transaction count', compute='_get_count')
    material_category = fields.Many2many('product.category', 'po_category_rel', 'purchase_order_id', 'category_id',
                                         string='Material Category')
    material = fields.Many2one('product.product', 'Material')
    from_date = fields.Date('From', default=str(datetime.now() + timedelta(days=-30)))
    ...
    order_line = fields.One2many('purchase.order.line', 'order_id', string='Order Lines',
                                 readonly="state != 'purchase' or state != 'done' or state != 'cancel'", copy=True,
                                 ondelete='cascade', store=True)
    requisition_id = fields.Many2one('purchase.requisition', string='Requisitions')
    type = fields.Selection([('international', 'International'), ('domestic', 'Domestic')], 'Type')
    ...
    po_requisition_line = fields.One2many('po.requisition', 'order_id', string='Requisition Lines', ondelete='cascade')
    ...
    def write(self, vals):
        if vals.get('po_requisition_line'):
            for i in vals.get('po_requisition_line'):
                ...
                purchase_order_obj = self.env['purchase.order'].browse(po_req_obj.order_id.id)
            ...
            for order_line in self.order_line:
                if order_line.delivery_schedule_line_ids:
                    ...
                    for schedule_line in order_line.delivery_schedule_line_ids:
                        quantity_to_deliver = quantity_to_deliver + schedule_line.quantity_to_deliver

                    if quantity_to_deliver != product_qty:
                        raise UserError(_('Quantity to Deliver should be equal Product Quantity.'))
    ...
    def _prepare_invoice(self):
        ...
        if self.project_id and self.project_wbs:
            ...
            res['project_wbs_id'] = self.project_wbs.id
    ...
    def get_seller_price(self, product_id):
        ...
        for seller_id in product_obj.seller_ids:
            if seller_id.id == self.partner_id.id:
    ...
    def onchange_po_requisition(self):
        ...
        for line in list(set(material_id_list)):
            po_requisition_obj = self.po_requisition_line.search([('material_id', '=', line.id)])
            for i in po_requisition_obj:
                vals = {
                    'product_id': line.id,
                    'product_uom': line.uom_id.id,
                    'price_unit': i.rate,
                    'product_qty': i.current_req_qty,
                    'name': line.name,
            ...
            for po_requisition_line in self.po_requisition_line:
                if order_lines.product_id.id == po_requisition_line.material_id.id:
                    ...
                    if po_requisition_line.specification:
                        specification = specification + ' ' + po_requisition_line.specification

            seller_price = self.get_seller_price(order_lines.product_id.id)
            received_qty = self.custom_compute_qty_received(order_lines.product_id.id)
            order_lines.update({
                'qty_received_custom': received_qty,
        ...
        if self.stage_id.approved == True:
            stage_master_obj = self.env['stage.master'].search([('amend_and_draft', '=', True)])
    ...
    def compute_po_lines(self, purchase_order_id, project_wbs, project):
        ...
        old_po_lines = [line.product_id.id for line in purchase_order_id.order_line]
        ...
        for line in compressed_material_list:
            ...
            for wbs in project_wbs_obj:
                material_estimate_obj = wbs.material_estimate_line.search([('material_id', '=', line.id)], limit=1)
        ...
        for order_lines in self.order_line:
            ...
            received_qty = order_lines.custom_compute_qty_received(order_lines.product_id.id)
        ...
        for line in purchase_order_id.order_line:
            product_obj = self.env['product.product'].search([('id', '=', line.product_id.id)])
            seller_cost = line.price_unit
            for seller_id in product_obj.seller_ids:
                if seller_id.id == purchase_order_id.partner_id.id and seller_id.is_active == True:
                    seller_cost = seller_id.price
                ...
                if budget_line.stage_id.approved == True:
                    ...
                    for po_line in self.po_requisition_line:
                        if po_line.task_id.category_id.id == budget_line.task_category.id:
                            total = (po_line.rate * po_line.current_order_qty) + total

                    if total > budget_line.materialbudget_remaining:
                        raise UserError(_('Insufficient Budget.'))
                    else:
                        budget_line.materialbudget_used = budget_line.materialbudget_used + total
        ...
        for order in self:
            if any([ptype in ['product', 'consu'] for ptype in order.order_line.mapped('product_id.type')]):
                ...
                moves = order.order_line.filtered(
                    lambda r: r.product_id.type in ['product', 'consu']).custom_create_stock_moves(picking)
    ...
    def cron_custom_create_picking(self):
        for order in self:
            if any([ptype in ['product', 'consu'] for ptype in order.order_line.mapped('product_id.type')]):
                ...
                picking = self.env['stock.picking'].create(res)
                moves = order.order_line.filtered(
                    lambda r: r.product_id.type in ['product', 'consu']).cron_custom_create_stock_moves(picking)
    ...
    def custom_create_stock_moves(self, picking):
        ...
        for line in self:
            ...
            for procurement in line.procurement_ids:
                procurement_qty = procurement.product_uom._compute_qty_obj(procurement.product_uom,
                                                                           procurement.product_qty, line.product_uom)
                tmp = template.copy()
                tmp.update({
    ...
    def confirm_shipment_on_cron(self):
        po_objs = self.env['purchase.order'].search([('state', '=', 'shipment')])
        for obj in po_objs:
    ...
    def onchange_project(self):
        ...
        # return required domain, i.e.wbs of only selected project and materials
        # of selected Wbs
        project_wbs_lst = []
...
class PurchaseOrderLine(models.Model):
    ...
    qty_received_custom = fields.Float('Received Qty', compute='_compute_qty_received', store=True)
    total_rate = fields.Float('Total Rate', help='This is rate of product including transportation and other charges',
                              compute='get_landed_cost', store=True)
    delivery_schedule_line_ids = fields.One2many('delivery.schedule', 'delivery_id', string='Delivery_Schedule_Line')
        ...
        for line in self:
            if line._context.get('project_id'):
                line.analytic_distribution = {
                    line.env['project.project'].browse(line._context['project_id']).analytic_account_id.id: 100}
                line.analytic_distribution = {
                    line.env['project.project'].browse(line._context['project_id']).analytic_account_id.id: 100}
            else:
    ...
    def get_landed_cost(self):
        for val in self:
            transport_amount = val.order_id.transport_amount + val.order_id.loading_charges + val.order_id.unloading_charges + val.order_id.other_charges
            if val.order_id.amount_total > 0:
                line_contribution = (transport_amount / val.order_id.amount_total) * val.price_total
    ...
    def custom_compute_qty_received(self, product_id):
        total = 0
        picking = self.env['stock.picking'].search([('state', '=', 'done'), ('po_id', '=', self.order_id.id),
...
class DeliverySchedule(models.Model):
    ...
    is_shipment_created = fields.Boolean("Is Shipment Created", default=False, readonly=True)
...
class POMaterialRate(models.Model):
    ...
    rate_master_id = fields.Many2one('material.rate', string='Rate master', required=True)
    material_rate_lines = fields.One2many('po.material.rate.line', 'material_rate_id')
...
class POMaterialRateLine(models.Model):
    ...
    net_rate = fields.Float(compute='_compute_amount', string='Net Rate', readonly=True, store=True)
    conversion_factor = fields.Float('Conversion Factor', default='1')
    unit_commercial = fields.Many2one('uom.uom', required=True)
    tax_id = fields.Many2many('account.tax', 'po_material_rate_line_tax_rel', 'tax_id', 'rate_line_id', string='Taxes')
    ...
    def onchange_name(self):
        journals = self.env['conversion.uom'].search([('prod_uom_id', '=', self.name.id)])
        vals = []
        for i in journals:
        ...
        return {
            'domain': {
                'unit_commercial': [('id', 'in', vals)],
                'brand_id': [('material_id', '=', self.name.id)]
            }
        }

    @api.onchange('supplier_id')
    def onchange_supplier_id(self):
        partner = self.env['res.partner'].search([('name', '=', self.supplier_id.name)])
        vals = []
        for i in partner:
    ...
    def _compute_amount(self):
        ...
        for line in self:
            ...
            taxes = line.tax_id.compute_all(price, line.currency_id, quantity=1.0, product=line.name,
...
class PORequisition(models.Model):
    ...
    stage_id = fields.Many2one('transaction.stage', domain=[('model', '=', 'requisition.order.line')], copy=False)
    project_wbs = fields.Many2one('project.task', 'Project Wbs')
    project_id = fields.Many2one('project.project', 'Project')

    sub_project = fields.Many2one('sub.project', 'Sub Project')

    material_category = fields.Many2one('product.category', 'Material Category')
    ...
    def write(self, vals):
        res = super(PORequisition, self).write(vals)
        return res

    def unlink(self):
        if self.order_id.state == 'shipment' or self.order_id.state == 'purchase' or self.order_id.state == 'done':
            raise UserError(('Cannot delete requisition line which is in shipment state.'))
        else:
            return super(PORequisition, self).unlink()
