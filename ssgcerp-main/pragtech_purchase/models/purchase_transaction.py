...
class PurchaseTransaction(models.Model):
    ...
    name = fields.Char('Transaction No.', readonly="state != 'draft'", copy=False, )
    project_id = fields.Many2one('project.project', string='Project', required=True,
                                 domain=[('approval_state', '=', 'approve')])
    project_wbs = fields.Many2one('project.task', 'project WBS Name',
                                  domain=[('is_wbs', '=', True), ('is_task', '=', False)], required=False)
    ...
    transaction_type = fields.Selection([('debit_note', 'Debit Note'), ('advance', 'Advance')],
    ...
    def on_change_work_order_id(self):
        order = self.env['purchase.order'].search([('id', '=', self.purchase_order_id.id)])
        for line in order:
    ...
    def create(self, vals_list):
        for vals in vals_list:
            rec = super(PurchaseTransaction, self).create(vals_list)
        ...
        if self.counter == 0:
            ...
            if context.get('copy') == True:
                ...
                if self.transaction_type == 'advance':
                    self.name = self.env['ir.sequence'].next_by_code('transaction.advance.purchase') or '/'
                    """updating advanced amount in WO"""

                if self.transaction_type == 'debit_note':
                    self.name = self.env['ir.sequence'].next_by_code('transaction.debit.purchase') or '/'
    ...
    def get_recovered_records_count(self):
        ...
        if self.transaction_type == 'advance':
            self.recovered_count = self.env['purchase.advance.line'].search_count([('advance_id', '=', self.id)])
        elif self.transaction_type == 'debit_note':
            self.recovered_count = self.env['purchase.debit.line'].search_count([('debit_id', '=', self.id)])
    ...
    def get_recoveries(self):
        ...
        if self.transaction_type == 'debit_note':
            ...
            recovery_line_obj = self.env['purchase.debit.line'].search([('debit_id', '=', self.id)])
            for line in recovery_line_obj:
                recovery_ids.append(line.debit_pur_line_id.id)

            view_id = self.env.ref('pragtech_purchase.purchase_advance_view_tree').id
            model = 'purchase.advance'
        elif self.transaction_type == 'advance':
            action = self.env.ref('pragtech_purchase.purchase_action_advance')
            recovery_line_obj = self.env['purchase.advance.line'].search([('advance_id', '=', self.id)])
            for line in recovery_line_obj:
