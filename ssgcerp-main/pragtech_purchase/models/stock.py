...
class StockPicking(models.Model):
    ...
    mrf_id = fields.Many2one('purchase.requisition', 'MRF')
        ...
        if context.get('copy') == True:
            ...
            for picking_line in self.move_ids:
                if picking_line.quantity == 0:
                    flag = 1
                    break
                elif picking_line.product_uom_qty < picking_line.quantity:
                    flag1 = 1
                    break
                else:
                    flag = 0
    ...
    project_wbs = fields.Many2one('project.task', 'Sub project',
                                  domain=[('is_wbs', '=', True), ('is_task', '=', False)], readonly=True)
    ...
    type = fields.Selection(
        [('from_supplier', 'Receipt from Supplier'), ('from_other_site', 'Receipt from Other site')], 'Type',
        readonly=True)
        ...
        for picking in self:
            ...
            for picking_line in picking.pack_operation_product_ids:
                ...
                purchase_line = self.env['purchase.order.line'].search(
                    [('order_id', '=', po_ids.id), ('product_id', '=', picking_line.product_id.id)])
...
class StockMove(models.Model):
    ...
    mrf_line_id = fields.Many2one('purchase.requisition.line', 'MRF Line')
