...
class VendorQuotation(models.Model):
    ...
    date_order = fields.Datetime('Quotation Date', required=True, select=True, copy=False,
    ...
    partner_id = fields.Many2one('res.partner', string='Vendor', required=True, change_default=True,
                                 track_visibility='always', readonly="state == 'confirm'")
    partner_ref = fields.Char('Reference', copy=False, readonly="state == 'confirm'",
    ...
    order_line = fields.One2many('vendor.quotation.line', 'order_id', string='Order Lines', copy=True,
                                 readonly="state == 'confirm'")
    ...
    valid_till = fields.Datetime('Valid Till', readonly="state == 'confirm'")
    transport_amount = fields.Float('Transport Amount', readonly="state == 'confirm'")
    loading_charges = fields.Float('Loading Charges', readonly="state == 'confirm'")
    unloading_charges = fields.Float('Unloading Charges', readonly="state == 'confirm'")
    other_charges = fields.Float('Other Charges', readonly="state == 'confirm'")
    payment_term_id = fields.Many2one('account.payment.term', 'Payment Term', readonly="state == 'confirm'")
    company_id = fields.Many2one('res.company', 'Company', index=1, default=lambda self: self.env.user.company_id.id,
                                 readonly="state == 'confirm'")
    use_in_quotation = fields.Boolean('Use In Quotation', readonly="state == 'confirm'")
    delivery_schedule = fields.Datetime(string='Delivery Schedule', readonly="state == 'confirm'")
    host_name = fields.Char(string='Host Name', readonly="state == 'confirm'")
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if vals.get('name', 'New') == 'New':
                vals['name'] = self.env['ir.sequence'].next_by_code('vendor.quotation') or '/'
            ...
            return super(VendorQuotation, self).create(vals_list)
    ...
    def unlink(self):
        ...
        Purchase_order_obj = self.env['quotation.details'].search([('name', 'in', list_ids)])
        if Purchase_order_obj:
            raise UserError(_('You cannot delete Quotation which is used in Quotation comparison.'))
...
class VendorQuotationLine(models.Model):
    ...
    def _default_category(self):
        category_obj = self.env['product.category'].search([('name', '=', 'All')], limit=1)
        if category_obj:
    ...
    taxes_id = fields.Many2many('account.tax', 'vendor_quot_line__tax_rel', 'tax_id', 'vendor_quot_line_id',
                                string='Taxes')
    product_uom = fields.Many2one('uom.uom', string='Unit', required=True)
    product_id = fields.Many2one('product.product', string='Material', change_default=True, required=True)
    ...
    order_id = fields.Many2one('vendor.quotation', string='Order Reference', select=True, required=True,
                               ondelete='cascade')
    ...
    product_category = fields.Many2one('uom.category', 'Material Category', required=True, default=_default_category)
    ...
    def _onchage_order_line(self):
        ...
        query = "select category_id from partner_prod_category_rel where partner_id={}".format(part.id)
