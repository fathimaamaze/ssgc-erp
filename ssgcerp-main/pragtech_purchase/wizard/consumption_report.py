...
class UnbilledGrnWizard(models.TransientModel):
    ...
    order_line = fields.One2many('consumption.report.lines.wizard', 'order_id', string='Order Lines', copy=True,
                                 ondelete='cascade')
    ...
    def compute_purchase_orders(self):
        ...
        view_id = self.env.ref('pragtech_purchase.consumption_report_wizard_form_view').id
        return {
...
class UnbilledGrnLinesWizard(models.TransientModel):
    ...
    order_id = fields.Many2one('consumption.report.wizard', string='Order Reference', ondelete='cascade')
    consumption_date = fields.Char("Consumption Date")
