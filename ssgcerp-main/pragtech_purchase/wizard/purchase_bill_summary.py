...
class POBillSummaryWizard(models.TransientModel):
    ...
    project_wbs_id = fields.Many2one('project.task', 'Project WBS',
    ...
    order_line = fields.One2many('po.bill.summary.lines.wizard', 'order_id', string='Order Lines', copy=True,
                                 ondelete='cascade')
    ...
    def onchange_project_id(self):
        if self.project_id:
            return {
                'domain': {
                    'project_wbs_id': [('project_id', '=', self.project_id.id), ('is_wbs', '=', True),
