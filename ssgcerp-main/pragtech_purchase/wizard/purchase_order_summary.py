...
class POSummaryWizard(models.TransientModel):
    ...
    order_line = fields.One2many('po.summary.lines.wizard', 'order_id', string='Order Lines', copy=True,
                                 ondelete='cascade')
    ...
    def compute_purchase_orders(self):
        ...
        view_id = self.env.ref('pragtech_purchase.purchase_order_summary_wizard_form_view').id
        return {
...
class POSummaryLinesWizard(models.TransientModel):
    ...
    order_id = fields.Many2one('po.summary.wizard', string='Order Reference', ondelete='cascade')
