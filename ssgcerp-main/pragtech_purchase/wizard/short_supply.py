...
class ShortSupplyWizard(models.TransientModel):
    ...
    order_line = fields.One2many('short.supply.lines.wizard', 'order_id', string='Order Lines', copy=True,
                                 ondelete='cascade')
    ...
    def compute_purchase_orders(self):
        ...
        view_id = self.env.ref('pragtech_purchase.purchase_order_short_supply_wizard_form_view').id
        return {
...
class ShortSupplyLinesWizard(models.TransientModel):
    ...
    order_id = fields.Many2one('short.supply.wizard', string='Order Reference', ondelete='cascade')
