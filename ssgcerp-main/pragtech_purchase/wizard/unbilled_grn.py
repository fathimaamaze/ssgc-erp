...
class UnbilledGrnWizard(models.TransientModel):
    ...
    order_line = fields.One2many('unbilled.grn.lines.wizard', 'order_id', string='Order Lines', copy=True,
                                 ondelete='cascade')
    ...
    def onchange_project_wbs(self):
        if self.project_wbs:
            return {
                'domain': {
                    'puchase_order_id': [('project_wbs', '=', self.project_wbs.id)],
                }
            }
    ...
    def compute_purchase_orders(self):
        ...
        view_id = self.env.ref('pragtech_purchase.purchase_order_unbilled_grn_wizard_form_view').id
        return {
...
class UnbilledGrnLinesWizard(models.TransientModel):
    ...
    _description = 'Un-billed GRN Lines'
    ...
    order_id = fields.Many2one('unbilled.grn.wizard', string='Order Reference', ondelete='cascade')
